...
<article id="siteContent">
	<div id="summary" class="mx-4">
		<div class="py-3 row">
			<h1 class="h2 text-center text-classic text-xxl py-2 col-md-8">
	...
	<div id="detail" class="container mx-4">
		<section id="teaser" class="row">
			...
			<div id="" class="col-lg-4 mb-2">
				<c:set var="tagContent"
					value="${ delivry.attributes.topo.available ? 'Disponible' : 'Réservé'}"
					scope="page"></c:set>
				<c:set var="tagType"
					value="${ delivry.attributes.topo.available ? 'success' : 'danger'}"
					scope="page"></c:set>
				<p class="jcm-tag-box">
					<span class="jcm-tag jcm-tag-${tagType} text-${tagType}">${tagContent}</span>
				</p>
				<div class="monkey-box monkey-box-${tagType}">
					<div class="monkey-hat monkey-hat-${tagType}"></div>
					<p class="monkey-drawer">
						Auteur :
						<c:out value="${ topo.writer }" />
					</p>
					<div class="monkey-separator"></div>
					<p class="monkey-drawer">
						Date d'édition :
						<c:out value="${ topo.writedAt }" />
					</p>
					<div class="monkey-separator"></div>
					<p id="description" class="monkey-drawer">
						<c:out value="${ delivry.attributes.topo.summary }" />
					</p>
				</div>
			</div>
		</section>
		<section id="content" class="row">
			<div id="topoDescription" class="desc-box col-xl-8">
				...
				<p class="desc-content">
					<c:out value="${ topo.content }" />
				</p>
			</div>
			<div id="topoLinks" class="col-xl-4">
				<div class="monkey-box monkey-box-classic">
					<h3 class="monkey-title monkey-title-classic text-white">Liens
						vers les spots</h3>
					<ul>
						...
						<c:forEach items="${ delivry.attributes.topo.sites }" var="site">
							...
							<li class="monkey-drawer"><a class="tex-classic"
